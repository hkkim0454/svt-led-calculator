...
| 7-b | 교육장 조명 · 제안 화각 · 카메라 높이 어긋남 원인 규명 | **작업 완료 (v441) · 오너 승인 대기** — 배치·가구·마감 변경 0 |
...
**측정 방법 메모** — 픽셀 기준선은 **같은 촬영 순서**로 찍어야 한다. LED 설치 높이는 공간
타입을 바꿔도 이어지는 입력값이라, 앞서 본 방에 따라 강당·아이디에이션의 LED 높이가 1.92m
와 2.08m 로 갈린다. v439 와 v441 에서 이 차이가 완전히 같으므로 코드 변화가 아니다.
