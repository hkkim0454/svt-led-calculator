...
## 교육장 · 트레이닝룸 V1 — 진행 중 (DEC-127, 2026-09-19)
...
측정으로 확인한 사실: **강의실이 회의실보다 전체적으로 밝은 가장 큰 이유는 마감이 아니라
조명이다.** 같은 도장 벽 색인데 벽이 223.4 vs 194.7 로 갈리는 것은 대기업이 `corporateSoft`
를 쓰고 강의실은 기준 조명을 그대로 쓰기 때문이다. 7-a 의 마감으로 위계와 클리핑은
바로잡았지만 전체 밝기 조정은 7-b 의 몫이다. 화각(실내 42° · 코너 40° · 눈높이 2.0~2.2m)도
공용값 그대로이고 7-b 에서 사람 눈높이 구도로 다룬다.
