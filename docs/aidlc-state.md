...
### 보고 — 임원 회의실 상태 불일치 (오너 판단 필요)

`executiveBoardroom` 은 PHASE 3-e에서 문서상 **RELEASE READY** 로 선언됐지만
데이터의 `status` 는 아직 `planned` 다. PHASE 4-e 사양서 §14가 '이번 단계가 돈다는 이유만으로
고치지 말라'고 했으므로 **이번에는 건드리지 않았다.** 메타데이터 한 줄 수정이므로
오너가 원하면 별도로 반영한다.
