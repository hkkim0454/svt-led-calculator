...
- Phase: **PHASE 10 — 제품 전체 시각 통일**. PHASE 9(강당 V1)까지 **완료**했고, PHASE 10-0 감사로 다음 트랙을
  골랐다. 10-a·10-b·10-c 는 병합했고, **10-d(대회의실·교육장 하단 띠)는 작업 중**이다.
- `main` 기준: **`c570963`** · 화면 버전 **v455** · `npm test` **1167/1167** · 작업 트리 clean.
- 작업 가지 `claude/phase10d-conference-training-bottom-band`: 화면 버전 **v456** · `npm test` **1169/1169**(신규 계약 2).
  **교육장 실내 한 컷의 서는 자리와 천장 띠만** 바꾼다. 대회의실은 기준과 기존 계약이 충돌해
  보류했고, 선택지를 `docs/aidlc-state.md` 의 DEC-150 절에 적어 두었다.
...
| 10-d 후속 | 대회의실 러그 — 바닥 회피 장치가 러그를 세지 않는 문제를 조사(구현 전 단계) |
...
- **배포(Production) 기준 주소 — GitHub Pages.** 오너가 실제 화면을 확인하는 곳이다.
  ...
  - 화면이 최신인지 보는 법: 페이지 아래쪽 `화면 버전 vNNN` 이 `src/index.html` 의 값과
    ...
    지금 `main` 기준값은 **v455**, 작업 가지 기준값은 **v456** 다.
