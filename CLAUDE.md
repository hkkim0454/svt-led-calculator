...
- Phase: **PHASE 10 — 제품 전체 시각 통일**. PHASE 9(강당 V1)까지 **완료**했고, PHASE 10-0 감사로 다음 트랙을
  골랐다. 10-a(문서·배포 기준)와 10-b(상황실 제안 카메라)는 병합했고,
  **10-c(아이디에이션 제안 카메라·밝은 면)는 작업 중**이다.
- `main` 기준: **`5b1906b`** · 화면 버전 **v454** · `npm test` **1160/1160** · 작업 트리 clean.
- 작업 가지 `claude/phase10c-ideation-camera-surface`: 화면 버전 **v455** · `npm test` **1166/1166**(신규 계약 6).
  아이디에이션 제안 세 컷의 구도와 라운지 좌석 마감만 바꾼다 — 배치·조명·LED·벽은 그대로다.
...
| **10-c** | 아이디에이션 제안 카메라와 밝은 면 — **작업 중**. 실내 LED 30.05% → 17.75% · 흰색 날림 1.366% → 0% |
...
- **배포(Production) 기준 주소 — GitHub Pages.** 오너가 실제 화면을 확인하는 곳이다.
  ...
  - 화면이 최신인지 보는 법: 페이지 아래쪽 `화면 버전 vNNN` 이 `src/index.html` 의 값과
    ...
    지금 `main` 기준값은 **v454**, 작업 가지 기준값은 **v455** 다.
