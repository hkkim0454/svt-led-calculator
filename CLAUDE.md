...
- Phase: **PHASE 10 — 제품 전체 시각 통일**. PHASE 9(강당 V1)까지 **완료**했고, PHASE 10-0 감사로 다음 트랙을
  골랐다. 10-a·10-b·10-c 는 병합했고, **10-d 는 부분 수용으로 병합**했다(PR #117 — 교육장만
  릴리스하고 대회의실은 보류). 이어서 **10-d.1 실현 가능성 감사를 마쳤다**(문서만 고쳤다).
- `main` 기준: **`823def9`** · 화면 버전 **v456** · `npm test` **1169/1169** · 작업 트리 clean.
- **대회의실은 DEFERRED 상태다.** 기준(하단 띠 55% 이하)과 기존 계약 ㉒·㉓ 이 충돌해 지금의 계약
  집합 안에서는 해가 없다. 경위와 측정값은 `docs/aidlc-state.md` 의 DEC-150 절과
  `PHASE 10-d.1 실현 가능성 감사` 절에 있다. **10-e 는 오너 승인 전까지 시작하지 않는다.**
...
| **10-b** | 상황실 제안 카메라 재설계 — **완료·병합**(PR #115). 바닥 30.7% → 6.6% · 화면 아래 띠 92.3% → 26.6%(실내) |
| **10-c** | 아이디에이션 제안 카메라와 밝은 면 — **완료·병합**(PR #116). 실내 LED 30.05% → 17.75% · 흰색 날림 1.366% → 0% |
| **10-d** | 대회의실·교육장 화면 아래 띠 — **부분 수용**(PR #117). 교육장 71.6% → 47.4% 릴리스, **대회의실은 DEFERRED**(기존 계약 ㉔㉕·㉗·§13·§14 유지, 미달 61.6/67.0/67.2% 는 accepted HOLD) |
| **10-d.1** | **완료** — Large Conference framing 실현 가능성 감사(문서만, 제품 변경 0건). `framingFields()` 가 러그를 카메라 풀이에 넘기지 않는 것은 **사실로 확인**했으나, 기준 방은 카메라가 이미 `rearMost` 자르기에 **포화**되어 있어 rug-aware framing 을 더해도 **깊이 11m 이하의 문제 방에서는 화면이 바뀌지 않는다(no-op)**. 실제 병목은 러그 미인식이 아니라 **하단 띠 55% 이하와 계약 ㉒(상판 점유) · ㉓(LED 담기) 사이의 충돌**이다. rug-aware solver 구현은 **DEFERRED**, 대회의실 Golden Reference 와 기존 계약은 **그대로 유지**한다. 근거는 `docs/aidlc-state.md` 의 `PHASE 10-d.1 실현 가능성 감사` 절에 있다 |
...
### 3D 구도를 잴 때의 기준 (PHASE 10-b, 2026-09-24)

**카메라 풀이는 사람이 입력한 LED 크기가 아니라, 캐비닛으로 맞춘 실제 LED 크기를 본다.**
`app.js` 가 `buildGLModel` 에 넘기는 값이 engine 의 `actualW`·`actualH` 이기 때문이다.
예를 들어 4,000×2,300 을 넣어도 MP012F 4×4 로 맞춰지면 실제 화면은 **3,226×1,814** 이고,
화면에 그려지는 것도 재어야 하는 것도 이쪽이다.

- 그래서 **탐사·측정 도구도 같은 길로 모델을 만들어야 한다** — `computeConfig()` 를 거쳐
  `actualW`/`actualH` 를 쓰고, 자세는 `settledPose()` 까지 통과시킨다. 그렇게 맞추면
  브라우저 실측과 0.2%p 안에서 일치한다(PHASE 10-b 대조).
- PHASE 10-b 이전의 탐사 도구는 **요청 크기(4,000×2,300)를 그대로 넣고 있었다.** 실제보다
  큰 LED 를 재고 있었다는 뜻이고, 그래서 이 단계에서 제품 경로 기준으로 다시 쟀다.
  옛 도구가 낸 구도 수치를 인용할 때는 이 점을 감안한다.
