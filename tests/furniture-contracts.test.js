...
test('avCredenza — 이미 있는 자산이고, 치수를 두 번 적지 않는다', () => {
  ...
  assert.deepEqual(existing, ['avCredenza']);
...
test('런타임 분리 — 계약이 런타임 카탈로그에 들어가지 않는다', () => {
  ...
  for (const id of CONTRACT_IDS) {
    if (FURNITURE_CONTRACTS[id].status === CONTRACT_STATUS.EXISTING) continue;
  ...
  assert.equal(FURNITURE_ASSETS.executiveChair, undefined);
  ...
  // 겹치는 이름은 '이미 있는 것'으로 표시된 avCredenza 하나뿐이다.
  const overlap = CONTRACT_IDS.filter(id => FURNITURE_ASSETS[id]);
  assert.deepEqual(overlap, ['avCredenza']);
});

test('기존 가구 무변경 — 카탈로그·치수·부품·묶음 열쇠가 그대로다', () => {
  // 기존 런타임 자산 14종이 그대로 있다.
  assert.deepEqual(Object.keys(FURNITURE_ASSETS), [
    'conferenceChair', 'auditoriumChair', 'trainingChair', 'trainingDesk', 'controlConsole',
    'podium', 'avCredenza', 'highTable', 'stool', 'loungeChair', 'collabTable',
  ...
  assert.equal(assetKey({ type: 'chair', asset: 'executiveChair' }), 'conferenceChair');
...
test('대기업 회의실 — 이번 단계에서도 디자인은 여전히 INHERIT 다', () => {
  // 계약을 만들었다고 해서 회의실 화면이 바뀌면 안 된다.
  assert.equal(ROOM_DESIGNS.corporateMeeting.furniture, null);
  assert.equal(ROOM_DESIGNS.corporateMeeting.materials, null);
  // 다만 PHASE 2 에서 쓸 계약은 준비되어 있다.
  assert.ok(hasFurnitureContract('corporateChair'));
  assert.ok(hasFurnitureContract('corporateTable'));
  assert.equal(FURNITURE_ASSETS.corporateChair, undefined, '아직 도형은 없다');
});
