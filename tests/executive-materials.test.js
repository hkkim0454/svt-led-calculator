...
test('⑰ 마감 단계가 조명·화각을 켠 것이 아니다 — 각각 3-d.1 · 3-d.2 에서 켰다', () => {
  const d = ROOM_DESIGNS[EX];
  assert.equal(d.lighting, 'executiveSoft');
  assert.equal(d.camera, 'executiveProposal');
...
test('㉑ 마감을 선언하지 않은 공간은 여전히 전부 null 이다', () => {
  for (const id of DESIGN_IDS) {
    // 대회의실은 PHASE 4-d.1, 상황실은 PHASE 5-d.1 에서 **제** 마감을 갖게 됐다.
    //   교육장은 PHASE 7-a 에서 제 마감을 갖게 됐다.
    if (id === 'corporateMeeting' || id === EX || id === 'largeConference'
      || id === 'controlRoom' || id === 'trainingRoom') continue;
