...
test('아직 구현하지 않은 1개 공간 — 적용해도 화면이 바뀌지 않는다(전부 INHERIT)', () => {
  ...
  // 상황실이 정한 것은 **의자와 콘솔 둘뿐**이다. 나머지(AV·마감·조명·화각)는 전부 planned 다.
  const ctrl = resolveDesign('controlRoom');
  assert.deepEqual(VALUE_FIELDS.flatMap(f => appliedIds(ctrl[f])), ['taskChair', 'curvedConsole'],
    '상황실이 의자·콘솔 말고 다른 것까지 정했다 (AV·마감·조명·화각은 PHASE 5-c 이후다)');
...
test('가짜 스펙 금지 — 아직 없는 자산은 planned로만 적히고 절대 적용되지 않는다', () => {
  ...
  //   대회의실의 벽 구성·소품은 아직 planned 다. 상황실은 의자·콘솔 말고 전부 planned 다.
  ...
  assert.deepEqual(applied.slice().sort(), [
    ...
    'conferenceProposal', 'conferenceSoft', 'corporateChair', 'corporateNeutral', 'corporateProposal', 'corporateSoft',
